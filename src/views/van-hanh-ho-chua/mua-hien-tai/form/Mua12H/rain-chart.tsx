...
const Mua12HourChart: React.FC<RainDataProps> = (props) => {
    const [data, setData] = useState<any[]>([])
    const {stationdata} = props;

    useEffect(() => {
      async function getDataTramQuangNgai() {
        await getData(`DuLieuTram/thong-ke-yeu-to-khi-tuong-12-gio/${stationdata.id}`)
          .then(data => {
            setData(data);
            console.log(data)
          })
